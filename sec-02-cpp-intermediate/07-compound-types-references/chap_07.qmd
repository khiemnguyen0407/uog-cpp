...
Why do we create an alias to a variable when we can just use the variable itself?

## Rationale: Recalling pass by value

We discussed [**pass by value**]{style="color:red"}, where an argument passed to a function is copied into the function's parameter.

::: {#lst-pass-by-value lst-cap="An example of pass by value Filename=`pass_by_value.cpp`"}
```{.cpp}
{{< include ./cpp/pass_by_value.cpp >}}
```
:::

## Rationale: Some objects are expensive to copy
...
- We want to avoid making unnecessary copies of objects that are expensive to copy, especially when these copied objects will be destroyed almost immediately.
:::
:::

:::{.fragment}
::: {#lst-pass-by-value-string lst-cap="Making a copy of `std::string` object is expensive. Filename=`pass_by_value_string.cpp`"}
```{.cpp}
{{< include ./cpp/pass_by_value_string.cpp >}}
```
:::
...
::: {.fragment}
::: {.incremental}
- To avoid making an expensive copy, we can use [**pass by reference**]{style="color:red"} instead of _pass by value_.
- To use [**pass by reference**]{style="color:red"}, we declare a function parameter as a _reference type_ (or _const reference type_) rather than as a normal type.
:::
:::

::: {.fragment}
::: {#lst-pass-by-reference lst-cap="To avoid expensive copy of arguments, use pass by reference. Filename=`pass_by_reference.cpp`"}
```{.cpp}
{{< include ./cpp/pass_by_reference.cpp >}}
```
:::

&#10149;&nbsp; When the function is called, each reference parameter is bound to the appropriate argument. **Note** _the ampersand in_ `std::string& y` _for the parameter declaration_.
:::

## Pass by reference: Key insight

::: {.my-box}
**Key insight**

[**Pass by reference**]{style="color:red"} allows us to pass arguments to a function [without making copies]{style="color:red"} of those arguments each time the function is called.
:::

&#10149;&nbsp; You don't believe me 😑😑😑 Let us run the following program in the next slide.


## Pass by reference: Examine the addresses

::: {#lst-pass-by-reference-address lst-cap="Mix of pass by value and pass by reference. Filename=`pass_by_value_vs_pass_by_reference.cpp`"}
```{.cpp}
{{< include ./cpp/pass_by_value_vs_pass_by_reference.cpp >}}
```
:::

## Pass by reference allows us to change the value of an argument

&#10149;&nbsp; Since a reference acts identically to the object being referenced, any changes made to the reference parameter will affect the argument.

::: {#lst-pass-by-ref-change-value lst-cap="Pass by reference allows us to change the value of an argument. Filename=`pass_by_reference_change_value.cpp`"}
```{.cpp}
{{< include ./cpp/pass_by_reference_change_value.cpp >}}
```
:::

&#10149;&nbsp; Let us run this code snippet.

## Pass by reference allows us to change the value of an argument: Key insight
...
Because a reference to a non-const value can only bind to a modifiable lvalue (essentially a non-const variable) &#10140; this means that [_pass by reference only works with arguments that are modifiable lvalues._]{style="color:navy"}

::: {#lst-pass-by-reference-accept-modifiable-lvalue lst-cap="Pass by reference can only accept modifiable lvalue arguments  Filename=`pass_by_reference_change_value.cpp`"}
```{.cpp}
{{< include ./cpp/pass_by_reference_accept_modifiable_lvalues.cpp >}}
```
:::

&#10149;&nbsp; Open the `cpp` file to see that IntelliSense informs the error before compilation.
...
::: {.incremental}
- We define a const object which is pretty expensive to make copy.
- We want to pass this const object as a function argument. 
- We also want to avoid making copy.
:::
:::
...
::: {#lst-pass-by-const-reference lst-cap="By making a reference parameter const, we can pass non-modifiable lvalues and rvalue literals. Filename=`pass_by_reference_change_value.cpp`"}
```{.cpp}
{{< include ./cpp/pass_by_reference_accept_modifiable_lvalues.cpp >}}
```
:::
...
- Passing by const reference offers the same primary benefit as pass by non-const reference (avoiding making a copy of the argument).
- The function can not change the value being referenced.

The following is not allowed:
```{.cpp}
void add_value(const double& ref, double value)
{
    ref = ref + value;  // not allowed.
                        // We promise the compiler that ref is const.
}
```

&#10149;&nbsp; Fortunately, in most cases we don't want our functions modifying the value of arguments. But we also don't want to make expensive copies. We are greedy😉😉!!!

## Pass by const lvalue reference: Best Practice
...
&#10149;&nbsp; Now, we can understand the motivation for allowing const lvalue references to bind to rvalues: 

&#10140;&nbsp; [Without that capability, there would be no way to pass literals (or other rvalues) to functions that used pass by reference!]{style="color:navy"}

# Return by reference

- When passing an argument by value, a copy of the argument is made into the function parameter.
- We can avoid making an expensive copy by passing by (const) reference (or pass by address) instead.
- A similar situation occurs when returning by value: [a copy of the return value is passed back to the caller]{style="color:blue"}

## Return by reference

&#9998;&nbsp; [**Return by reference**] returns a reference that is bound to the object being returned, which avoids making a copy of the return.

&#9998;&nbsp; To return by reference, we define the return value of the function to be a reference type
...
const std::string& return_by_reference();
```

## Return by reference: An academic example

::: {#lst-return-by-reference lst-cap="An example of return by reference. The variable `str_program_name` is made static so that it survives throughout the entire program. Filename=`return_by_ref_academic.cpp`"}
```{.cpp}
{{< include ./cpp/return_by_ref_academic.cpp >}}
```
:::
...
- Otherwise, the reference being returned will be left dangling (referencing an object that has been destroyed)

## Object being returned by reference must exist after the function returns
&#10149;&nbsp; Look at this example \& run it.

::: {#lst-return-by-reference-lifetime lst-cap="The object being returned by reference must exist after the function returns. Filename=`return_by_ref_lifetime.cpp`"}
```{.cpp}
{{< include ./cpp/return_by_ref_lifetime.cpp >}}
```
:::

## Return by reference: A lengthy but simple example

::: {#lst-return-by-reference-example lst-cap="A lengthy but simple example of return by reference Filename=`return_by_ref_example.cpp`"}
```{.cpp}
{{< include ./cpp/return_by_ref_example.cpp >}}
```
:::
